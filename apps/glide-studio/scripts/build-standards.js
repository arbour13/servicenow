 ...
 *
 * Two authoring conventions the renderer understands, both feeding the doc<->builder connection:
 *   - Fenced code tagged with a builder key -> a "Build this in <mode>" button under the block,
 *     signalling the example IS the app's real generated output:  ```js build=GlideAggregate
 *   - An h2 section annotated with `<!-- build: ScriptInclude -->` (comma-separated for several)
 *     on its own line -> the same button row under the section heading.
 * Both emit `<button class="std-build" data-build="<ModeKey>">`; a single delegated click handler
 * on the Standards page (main.controller.js vm.onStandardsClick) turns any of them into a
 * vm.openBuilder(<ModeKey>). data-build values are the exact mode keys vm.selectMode expects.
 */
'use strict';
...
// Builder mode keys (must match main.controller.js vm.modes keys / vm.selectMode) -> display
// names, for the "Build this in <name>" buttons. Any build= tag or `<!-- build: -->` value not in
// this map throws at build time, so a typo can't silently produce a dead button.
var MODE_NAMES = {
...
// One "Build this in <mode>" button. Throws on an unknown mode key so a bad build= tag fails the
// build instead of shipping a dead button.
function buildButton(modeKey) {
  var name = MODE_NAMES[modeKey];
  if (!name) { throw new Error('Unknown build mode key: "' + modeKey + '" (expected one of ' + Object.keys(MODE_NAMES).join(', ') + ')'); }
  return '<button type="button" class="std-build" data-build="' + modeKey + '">Build this in ' + name + ' →</button>';
}
...
function renderBlocks(lines, slugs) {
  ...
  while (i < lines.length) {
    ...
    if (/^```/.test(line)) {
      // Opening fence may carry an info string: ```js build=GlideAggregate  (the build= key marks
      // the block as the app's real generated output and gets a "Build this in <mode>" button).
      var buildMatch = line.match(/build=([A-Za-z]+)/);
      var code = [];
      i++;
      while (i < lines.length && !/^```/.test(lines[i])) { code.push(lines[i]); i++; }
      i++; // closing fence
      html.push('<pre class="std-pre"><code>' + highlight(code.join('\n')) + '</code></pre>');
      if (buildMatch) { html.push('<div class="std-build-row">' + buildButton(buildMatch[1]) + '</div>'); }
      continue;
    }
...
// Pulls `<!-- build: ModeA, ModeB -->` annotation lines out of a section's body (they attach a
// "Build this in <mode>" button to the section as a whole, distinct from a code-block build= tag).
// Returns { modes, lines } with the annotation lines removed so they never reach renderBlocks.
function extractBuildModes(lines) {
  var modes = [];
  var kept = [];
  lines.forEach(function (line) {
    var m = line.match(/^<!--\s*build:\s*([^>]+?)\s*-->\s*$/);
    if (m) {
      m[1].split(',').forEach(function (k) { var key = k.trim(); if (key) { modes.push(key); } });
    } else {
      kept.push(line);
    }
  });
  return { modes: modes, lines: kept };
}
    ...
    parts: parts.map(function (part) {
      ...
      return {
        ...
        sections: part.sections.map(function (section) {
          ...
          var b = extractBuildModes(section.lines);
          var src = extractSource(b.lines);
          var buildRow = b.modes.length
            ? '<div class="std-build-row">' + b.modes.map(buildButton).join('') + '</div>\n'
            : '';
          var sourceBadge = src.source
            ? ' <span class="std-source-badge" title="Beyond GlideFast\'s original three guides">Glide Studio addition</span>'
            : '';
          return {
            id: id,
            title: section.title,
            html: '<h2 id="std-' + id + '">' + renderInline(section.title) + sourceBadge + '</h2>\n' + buildRow + renderBlocks(src.lines, slugs),
          };
        }),
      };
    }),
