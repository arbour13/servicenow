/* Encoded Query codegen. Groups are joined with a plain '^' (AND), matching the UI's own
   labeling (group-join "AND" pill, "+ Add condition group (AND)" button, hint text). An earlier
   version joined subsequent groups with '^NQ' (ServiceNow's "New Query" operator), which is a
   union/OR of independent query segments, not an AND - a real bug, since fixed. */
angular.module('glideStudio').factory('EncoderService', function () {
  'use strict';
