/* Table list and field-name lookups for every gs-select table/field picker across all modes.
   Offline (hardcoded) tables/fields by default, swapped to the live-only set once actually connected
   (see SchemaService's own comment on why the two are kept separate) - listens for
   ConnectionUiService's 'gs:connectionChanged' broadcast to know when to swap. Pulled out of
   MainController, which passes vm.tables/vm.tableLabel/vm.fieldNames/vm.ensureFieldsLoaded/
   vm.searchTables straight through to this service - the template's existing bindings needed no
   changes.

   vm.tables mutates svc.tables IN PLACE (length=0 + push) rather than ever reassigning it, matching
   this app's other isolate-`=`-bound option lists (see main.controller.js's own
   vm.multiActionOptions comment) - gs-select's gs-options="vm.tables" binding dirty-checks by
   reference, and reassigning would desync the controller's one-time `vm.tables = SchemaUiService.tables`
   reference copy from whatever this service points to afterward. */
angular.module('glideStudio').factory('SchemaUiService', [
