# GlideFast Docs - Fluent (Now SDK) project

Generated by the SN Deployment Packager. **These are the files the Now SDK needs to deploy this app.**
The zip is a full ServiceNow Now SDK project: Service Portal widget, Angular providers, page, portal, and theme as metadata-as-code. Reinstalls update the same records via stable sys_ids.
...
- `src/fluent/tables/` - Fluent `Table()` definitions (x_gf_docs_group, x_gf_docs_page)
