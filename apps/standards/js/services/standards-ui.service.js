/* The Standards document's DOM-heavy mechanics: the deep-link scroll-to-section animation and the
   "on this page" rail's scroll-spy (IntersectionObserver). Single-instance UI (there's only one
   Standards pane), so unlike gs-select/gs-condition-groups/gs-modal this isn't a directive - the
   only argument for one would be "needs a real DOM handle at link time," not genuine reuse across
   multiple elements, so it stays a plain service like the app's other UI-state extractions.

   vm.stdFocusId/vm.activeStdId stay OWNED BY MainController (read in 11+ template spots as plain
   `vm.stdFocusId`, not through a function call) - this service never touches them directly, it
   reports changes back through the onFocusChange/onActiveChange callbacks scrollTo()/
   setupScrollSpy() take, and the controller writes them onto vm itself. Mode-routing (vm.activeMode,
   vm.selectMode, vm.openStandard/vm.openBuilder/vm.onStandardsClick) also stays in the controller -
   this service only does what happens once you're already looking at the Standards page, not how
   you got there. */
angular.module('standardsPortal').factory('StandardsUiService', ['$rootScope', '$timeout', 'StandardsService', '$sce', function ($rootScope, $timeout, StandardsService, $sce) {
  'use strict';
