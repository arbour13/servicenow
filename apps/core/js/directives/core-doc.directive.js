/* <core-doc doc="vm.doc"></core-doc>

   Generic documentation/wiki viewer UI (Core). Self-contained: it renders a doc-data object (see
   DocViewerService for the shape) as a searchable, navigable document - a sticky "on this page"
   rail beside the content, live search filtering, click-to-scroll, and an IntersectionObserver
   scroll-spy that highlights the section currently at the top of the viewport. All DOM handling is
   scoped to this element (no global selectors), so any number of these can coexist and any app can
   reuse it by feeding its own content. Styling lives in the host widget's own scss (`.doc-*`). */
angular.module('core').directive('coreDoc', ['DocViewerService', '$timeout', function (DocViewerService, $timeout) {
  return {
    restrict: 'E',
    scope: { rawDoc: '=doc' },
    template:
