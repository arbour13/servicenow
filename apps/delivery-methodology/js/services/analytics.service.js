/* Usage Insights (Service Portal UI analytics) bridge.

   Wraps the platform snAnalytics client when the portal analytics plugin is present; no-ops in the
   local harness and on instances without the plugin. Controllers/services call the typed helpers
   below - do not put event writes on the content table or through LiveSync.

   Event names are stable product vocabulary for Usage Insights dashboards (Phase 2). Keep the set
   small; add a new name only when a real KPI needs it. */
angular.module('deliveryMethodology').factory('AnalyticsService', [
