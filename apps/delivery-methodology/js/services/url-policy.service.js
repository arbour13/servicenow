/* Editor URL policy for templates + in-memory normalize (blur / save / load).

   The allowlist / instance-strip rules also live in js/lib/url-policy.js (DMUrlPolicy), which the
   packager concatenates onto the widget server script so dehydrate can sanitize on save. The
   harness loads that lib as a <script>; the deployed SPAngularProvider cannot (type:script is
   harness-only), so this factory embeds the same rules and prefers the shared global when present.
   Keep the embedded copy aligned with js/lib/url-policy.js. */
angular.module('deliveryMethodology').factory('UrlPolicyService', [function () {
  'use strict';
