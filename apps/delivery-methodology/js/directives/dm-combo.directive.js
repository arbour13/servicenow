/* Text field with a dropdown of common values. The field stays free text - the list is a shortcut,
   never a constraint - so this is deliberately not a <select>.

   It is also deliberately not a <datalist>, which is what this replaced. Chrome filters datalist
   options against whatever is already in the field, so on a pre-populated field the list filters to
   nothing and the popup silently refuses to open. Since every field this control is used on arrives
   with a value in it, the native version helped almost never. */
angular.module('deliveryMethodology').directive('dmCombo', ['$document', '$timeout', function ($document, $timeout) {
  'use strict';
